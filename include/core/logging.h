...
being called.
        * Version 1.3.1 - 2016-07-20 - Add LOGGING_UNSAFE_SIGNAL_HANDLER to
...
logging::create_directories
        * Version 1.4.1 - 2016-09-29 - Customize formating with
LOGGING_FILENAME_WIDTH
        * Version 1.5.0 - 2016-12-22 - LOGGING_USE_FMTLIB by kolis and
LOGGING_WITH_FILEABS by scinart
        * Version 1.5.1 - 2017-08-08 - Terminal colors on Windows 10 thanks to
looki
        * Version 1.6.0 - 2018-01-03 - Add LOGGING_RTTI and LOGGING_STACKTRACES
settings
...
LOGGING_VERBOSE_SCOPE_ENDINGS, add LOGGING_SCOPE_TIME_PRECISION, add named log
levels
        * Version 2.0.0 - 2018-09-22 - Split logging.hpp into logging.hpp and
logging.cpp

# Compiling
        Just include <logging.hpp> where you want to use Loguru.
        Then, in one .cpp file #include <logging.cpp>
        Make sure you compile with -std=c++11 -lstdc++ -lpthread -ldl

# Usage
        For details, please see the official documentation at
emilk.github.io/logging
        ...
        int main(int argc, char* argv[]) {
                ...
                logging::add_file("everything.log", logging::Append,
logging::Verbosity_MAX);
...
#if defined(LOGGING_IMPLEMENTATION)
#warning \
    "You are defining LOGGING_IMPLEMENTATION. This is for older versions of Loguru. You should now instead include logging.cpp (or build it and link with it)"
#endif
...
// LOGGING_VERSION_MAJOR, LOGGING_VERSION_MINOR, LOGGING_VERSION_PATCH);
#define LOGGING_VERSION_MAJOR 2
...
#ifndef LOGGING_SCOPE_TIME_PRECISION
// Resolution of scope timers. 3=ms, 6=us, 9=ns
#define LOGGING_SCOPE_TIME_PRECISION 3
#endif
...
#ifndef LOGGING_VERBOSE_SCOPE_ENDINGS
// Show milliseconds and scope name at end of scope.
#define LOGGING_VERBOSE_SCOPE_ENDINGS 1
#endif
...
LOGGING_EXPORT
Text textprintf(LOGGING_FORMAT_STRING_TYPE format, ...)
    LOGGING_PRINTF_LIKE(1, 2);
...
*/
LOGGING_EXPORT extern Verbosity g_stderr_verbosity;
LOGGING_EXPORT extern bool g_colorlogtostderr;  // True by default.
LOGGING_EXPORT extern unsigned
    g_flush_interval_ms;  // 0 (unbuffered) by default.
LOGGING_EXPORT extern bool
    g_preamble;  // Prefix each log line with date, time etc? True by default.

// Turn off individual parts of the preamble
LOGGING_EXPORT extern bool g_preamble_date;    // The date field
LOGGING_EXPORT extern bool g_preamble_time;    // The time of the current day
LOGGING_EXPORT extern bool g_preamble_uptime;  // The time since init call
LOGGING_EXPORT extern bool g_preamble_thread;  // The logging thread
LOGGING_EXPORT extern bool
    g_preamble_file;  // The file from which the log originates from
LOGGING_EXPORT extern bool g_preamble_verbose;  // The verbosity field
LOGGING_EXPORT extern bool
...
/*  Should be called from the main thread.
        ...
        logging::init() will look for arguments meant for logging and remove
   them. Arguments meant for logging are: -v n   Set logging::g_stderr_verbosity
   level. Examples: -v 3        Show verbosity level 3 and lower. -v 0 Only show
   INFO, WARNING, ERROR, FATAL (default). -v INFO     Only show INFO, WARNING,
   ERROR, FATAL (default). -v WARNING  Only show WARNING, ERROR, FATAL. -v ERROR
   Only show ERROR, FATAL. -v FATAL    Only show FATAL. -v OFF      Turn off
   logging to stderr.
   ...
   -v flag. Note that -v does not affect file logging (see logging::add_file).
...
// Returns all arguments given to logging::init(), but escaped with a single
// space as separator.
LOGGING_EXPORT
...
/*  Will log to a file at the given path.
        ...
        To stop the file logging, just call logging::remove_callback(path) with
   the same path.
*/
LOGGING_EXPORT
...
void log(Verbosity verbosity, const char* file, unsigned line,
         LOGGING_FORMAT_STRING_TYPE format, fmt::ArgList args);
FMT_VARIADIC(void, log, Verbosity, const char*, unsigned,
             LOGGING_FORMAT_STRING_TYPE)
...
LOGGING_EXPORT
void log(Verbosity verbosity, const char* file, unsigned line,
         LOGGING_FORMAT_STRING_TYPE format, ...) LOGGING_PRINTF_LIKE(4, 5);
...
class LOGGING_EXPORT LogScopeRAII {
public:
    ...
    LogScopeRAII(LogScopeRAII&& other)
        ...
        for (unsigned int i = 0; i < LOGGING_SCOPE_TEXT_SIZE; ++i) {
...
// log_and_abort.
LOGGING_EXPORT
LOGGING_NORETURN void log_and_abort(int stack_trace_skip, const char* expr,
...
/*  Add a string to be replaced with something else in the stack output.
   ...
   logging::demangle(typeid(std::ofstream).name());
                logging::add_stack_cleanup(verbose_type_name.c_str();
...
// Colors
LOGGING_EXPORT const char* terminal_black();
LOGGING_EXPORT const char* terminal_red();
LOGGING_EXPORT const char* terminal_green();
LOGGING_EXPORT const char* terminal_yellow();
LOGGING_EXPORT const char* terminal_blue();
LOGGING_EXPORT const char* terminal_purple();
LOGGING_EXPORT const char* terminal_cyan();
LOGGING_EXPORT const char* terminal_light_gray();
LOGGING_EXPORT const char* terminal_light_red();
LOGGING_EXPORT const char* terminal_white();

// Formating
LOGGING_EXPORT const char* terminal_bold();
LOGGING_EXPORT const char* terminal_underline();

// You should end each line with this!
LOGGING_EXPORT const char* terminal_reset();
...
class LOGGING_EXPORT EcEntryBase {
public:
...
/* 	A stack trace gives you the names of the function at the point of a
   ...
   variables. Usage:
        ...
        Use logging::get_error_context() to get the contents of the active error
   contexts.
...
#define ERROR_CONTEXT(descr, data)                                          \
    const logging::EcEntryData<logging::make_ec_type<decltype(data)>::type> \
        LOGGING_ANONYMOUS_VARIABLE(error_context_scope_)(                   \
            ...
            static_cast<logging::EcEntryData<                               \
                logging::make_ec_type<decltype(data)>::type>::Printer>(     \
...
/*
   ...
   error context stack. You can pass the handle to logging::get_error_context on
...
LOGGING_EXPORT Text ec_to_text(const char* data);
LOGGING_EXPORT Text ec_to_text(char data);
LOGGING_EXPORT Text ec_to_text(int data);
LOGGING_EXPORT Text ec_to_text(unsigned int data);
LOGGING_EXPORT Text ec_to_text(long data);
LOGGING_EXPORT Text ec_to_text(unsigned long data);
LOGGING_EXPORT Text ec_to_text(long long data);
LOGGING_EXPORT Text ec_to_text(unsigned long long data);
LOGGING_EXPORT Text ec_to_text(float data);
LOGGING_EXPORT Text ec_to_text(double data);
LOGGING_EXPORT Text ec_to_text(long double data);
LOGGING_EXPORT Text ec_to_text(EcHandle);
...
#define LOG_F(verbosity_name, ...) \
    VLOG_F(logging::Verbosity_##verbosity_name, __VA_ARGS__)
...
#define VLOG_SCOPE_F(verbosity, ...)                                        \
    logging::LogScopeRAII LOGGING_ANONYMOUS_VARIABLE(error_context_RAII_) = \
        ((verbosity) > logging::current_verbosity_cutoff())                 \
            ...
            : logging::LogScopeRAII(verbosity, __FILE__, __LINE__,          \
                                    __VA_ARGS__)
...
#define RAW_LOG_F(verbosity_name, ...) \
    RAW_VLOG_F(logging::Verbosity_##verbosity_name, __VA_ARGS__)
...
// Message is optional
#define ABORT_F(...) \
    logging::log_and_abort(0, "ABORT: ", __FILE__, __LINE__, __VA_ARGS__)

// --------------------------------------------------------------------
// CHECK_F macros:

#define CHECK_WITH_INFO_F(test, info, ...)                                 \
    LOGGING_PREDICT_TRUE((test) == true)                                   \
        ? (void)0                                                          \
        : logging::log_and_abort(0, "CHECK FAILED:  " info "  ", __FILE__, \
...
LOGGING_EXPORT
std::string strprintf(LOGGING_FORMAT_STRING_TYPE format, ...)
    LOGGING_PRINTF_LIKE(1, 2);

// Like vsprintf, but returns the formated text.
LOGGING_EXPORT
std::string vstrprintf(LOGGING_FORMAT_STRING_TYPE format, va_list)
    LOGGING_PRINTF_LIKE(1, 0);

class LOGGING_EXPORT StreamLogger {
public:
...
class LOGGING_EXPORT AbortLogger {
public:
...
#define DEFINE_CHECK_OP_IMPL(name, op)                                         \
    ...
    inline std::string* name(const char* expr, const T1& v1,                   \
        ...
        if (LOGGING_PREDICT_TRUE(v1 op v2)) {                                  \
            return NULL;                                                       \
        }                                                                      \
...
#define VLOG_IF_S(verbosity, cond)                                         \
    ((verbosity) > logging::current_verbosity_cutoff() || (cond) == false) \
        ? (void)0                                                          \
        : logging::Voidify() &                                             \
              logging::StreamLogger(verbosity, __FILE__, __LINE__)
#define LOG_IF_S(verbosity_name, cond) \
    VLOG_IF_S(logging::Verbosity_##verbosity_name, cond)
...
#define ABORT_S() \
    logging::Voidify() & logging::AbortLogger("ABORT: ", __FILE__, __LINE__)
...
#define CHECK_WITH_INFO_S(cond, info)                                        \
    ...
    ? (void)0                                                                \
    : logging::Voidify() & logging::AbortLogger("CHECK FAILED:  " info "  ", \
                                                __FILE__, __LINE__)
...
#define CHECK_OP_S(function_name, expr1, op, expr2)                            \
    while (auto error_string = logging::function_name(                         \
               #expr1 " " #op " " #expr2, logging::referenceable_value(expr1), \
               #op, logging::referenceable_value(expr2)))                      \
...
#define DVLOG_IF_S(verbosity, cond)                               \
    (true || (verbosity) > logging::current_verbosity_cutoff() || \
     (cond) == false)                                             \
        ? (void)0                                                 \
        : logging::Voidify() &                                    \
              logging::StreamLogger(verbosity, __FILE__, __LINE__)
...
#define DLOG_IF_S(verbosity_name, cond) \
    DVLOG_IF_S(logging::Verbosity_##verbosity_name, cond)
