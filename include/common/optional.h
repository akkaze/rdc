...
RDC_DECLARE_TYPE_NAME(optional<int>, "int or None");
/*! @brief description for optional bool */
RDC_DECLARE_TYPE_NAME(optional<bool>, "boolean or None");
/*! @brief description for optional float */
RDC_DECLARE_TYPE_NAME(optional<float>, "float or None");
/*! @brief description for optional double */
RDC_DECLARE_TYPE_NAME(optional<double>, "double or None");
...
template <typename T>
struct hash<rdc::optional<T>> {
    /*!
     * @brief returns hash of the optional value.
     * @param val value.
     * @return hash code.
     */
    size_t operator()(const rdc::optional<T>& val) const {
        std::hash<bool> hash_bool;
        size_t res = hash_bool(val.has_value());
        if (val.has_value()) {
            res = rdc::HashCombine(res, val.value());
        }
        return res;
    }
};
