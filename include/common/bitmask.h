...
template <class T>
struct has_max_element<T, void_t<decltype(T::_bitmask_max_element)>>
    : std::true_type {};
...
#define bitmask_constexpr_assert(cond)                                  \
    ((void)((cond) ? 0                                                  \
                   : (bitmask::bitmask_detail::constexpr_assert_failed( \
                          []() { assert(!#cond); }),                    \
                      0)))
...
template <class T>
inline constexpr bitmask_detail::underlying_type_t<T> get_enum_mask(
    const T&) noexcept {
    static_assert(
        bitmask_detail::is_valid_enum_definition<T>::value,
...
public:
    ...
    constexpr bitmask(value_type value) noexcept
        : m_bits{bitmask_detail::checked_value(
...
template <class T>
inline constexpr bitmask_detail::underlying_type_t<T> bits(
    const bitmask<T>& bm) noexcept {
    return bm.bits();
}
...
struct hash<bitmask::bitmask<T>> {
    constexpr std::size_t operator()(const bitmask::bitmask<T>& op) const
        noexcept {
...
#define BITMASK_MAKE_UNIQUE_NAME(prefix) \
    BITMASK_DETAIL_CONCAT(prefix, __COUNTER__)
#endif

#define BITMASK_DETAIL_DEFINE_OPS(value_type)                                 \
    inline constexpr bitmask::bitmask<value_type> operator&(                  \
    ...
    inline constexpr bitmask::bitmask<value_type>::underlying_type bits(      \
        value_type op) noexcept {                                             \
    ...
    class BITMASK_MAKE_UNIQUE_NAME(_disable_unused_function_warnings_) {      \
        static constexpr int _unused() noexcept {                             \
            return bitmask::bitmask_detail::disable_unused_function_warnings< \
                       value_type>(),                                         \
                   0;                                                         \
        }                                                                     \
    };                                                                        \
...
#define BITMASK_DETAIL_DEFINE_VALUE_MASK(value_type, value_mask)            \
    inline constexpr bitmask::bitmask_detail::underlying_type_t<value_type> \
...
#define BITMASK_DETAIL_DEFINE_MAX_ELEMENT(value_type, max_element)          \
    inline constexpr bitmask::bitmask_detail::underlying_type_t<value_type> \
    get_enum_mask(value_type) noexcept {                                    \
        return bitmask::bitmask_detail::mask_from_max_element<              \
...
#define BITMASK_DEFINE(value_type) BITMASK_DETAIL_DEFINE_OPS(value_type)
